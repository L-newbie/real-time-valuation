...
export interface SinaEstimateResult {
...
export async function fetchPrevNav(fundCode: string): Promise<PrevNavInfo | null> {
  const r = await fetchSinaEstimateWithRetry(fundCode)
  return r?.prevNav ?? null
}

export async function fetchFundgz(fundCode: string): Promise<FundValuation | null> {
  const r = await fetchSinaEstimateWithRetry(fundCode)
  return r?.valuation ?? null
}

async function fetchSinaEstimateWithRetry(fundCode: string): Promise<SinaEstimateResult | null> {
...
async function fetchSinaEstimate(fundCode: string): Promise<SinaEstimateResult | null> {
  ...
  const jzrq = last.pre_date || ''

  const valuation: FundValuation = {
    fundcode: fundCode,
  ...
  return {
    valuation,
    prevNav: jzrq ? { dwjz: gz, jzrq } : null,
  }
}
