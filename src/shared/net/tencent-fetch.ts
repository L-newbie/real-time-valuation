...
import { classifyUSSessionByMs } from '@/shared/market/session'
...
export async function fetchTencentRealtimeBatch(
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    const ext = extractExtendedHours(parts, entry.market, price)
    result.set(entry.code, { price, prevClose, changeRate, ...(name ? { name } : {}), ...ext })
  }
  return result
}

export function usExtendedSession(ms: number = Date.now()): 'PRE' | 'REGULAR' | 'POST' {
  const s = classifyUSSessionByMs(ms)
  if (s === 'PRE') return 'PRE'
  if (s === 'REGULAR') return 'REGULAR'
  return 'POST'
}

function extractExtendedHours(
  parts: string[],
  market: StockMarket,
  regularPrice: number,
): { extPrice?: number; extRate?: number } {
  if (market !== 'US') return {}
  ...
  const extPrice = parseFloat(parts[9])
  if (!Number.isFinite(extPrice) || extPrice <= 0) return {}

  if (Math.abs(extPrice - regularPrice) / regularPrice > 0.25) return {}

  return {
    extPrice,
    extRate: Math.round((extPrice - regularPrice) / regularPrice * 100 * 100) / 100,
  }
}
