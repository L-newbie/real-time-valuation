    ...
    <div ref="detailBodyEl" class="detail-body" @scroll.passive="onBodyScroll">
      ...
      <section class="dp-panel">
                  ...
                  <div v-if="expandedStocks.has(stock.stockCode)" class="hx-detail">
                    <div class="hx-detail-clip">
                      ...
                      <template v-if="stockDetails.get(stock.stockCode)">
                        ...
                        <dl class="hx-rows">
                          <div v-if="stockDetails.get(stock.stockCode)!.priceLine" class="hx-row">
                            <dt>价格</dt>
                            <dd class="font-number">{{ stockDetails.get(stock.stockCode)!.priceLine }}</dd>
                          </div>
                          <div v-if="stockDetails.get(stock.stockCode)!.calcLine" class="hx-row">
                            <dt>涨跌算式</dt>
                            <dd class="font-number">{{ stockDetails.get(stock.stockCode)!.calcLine }}</dd>
                          </div>
                          <div class="hx-row">
                            <dt>交易状态</dt>
        ...
        <template v-else-if="activeTab === 'records'">
          <OperationRecords :fund-code="fundCode" />
        </template>
        <template v-else>
          <div v-if="detailLoading" class="dp-empty"><span class="animate-breathe">加载中...</span></div>
...
import OperationRecords from '@/components/shared/operation-records.vue'
...
const SESSION_CN: Record<string, string> = { PRE: '盘前', REGULAR: '盘中', POST: '盘后' }

function fmtPrice(v: number | undefined): string {
  return v != null && Number.isFinite(v) ? v.toFixed(2) : '--'
}

function buildPriceLine(q?: StockQuoteInfo): string {
  if (!q) return ''
  const segs: string[] = []
  if (q.prevClose != null) segs.push(`昨收 ${fmtPrice(q.prevClose)}`)
  if (q.price != null) segs.push(`收盘 ${fmtPrice(q.price)}`)
  if (q.extPrice != null) segs.push(`${SESSION_CN[q.session ?? ''] ?? '盘外'} ${fmtPrice(q.extPrice)}`)
  return segs.join(' · ')
}

function buildCalcLine(q?: StockQuoteInfo): string {
  if (!q || q.changeRate == null) return ''
  if (q.extPrice != null && q.price != null && q.price > 0) {
    const label = SESSION_CN[q.session ?? ''] ?? '盘外'
    return `${label}：(${fmtPrice(q.extPrice)} − ${fmtPrice(q.price)}) ÷ ${fmtPrice(q.price)} = ${q.changeRate.toFixed(2)}%`
  }
  if (q.price != null && q.prevClose != null && q.prevClose > 0) {
    return `盘中：(${fmtPrice(q.price)} − ${fmtPrice(q.prevClose)}) ÷ ${fmtPrice(q.prevClose)} = ${q.changeRate.toFixed(2)}%`
  }
  return ''
}
...
const stockDetails = computed(() => {
  const out = new Map<string, {
    ...
    priceLine: string; calcLine: string
  }>()
  ...
  for (const stock of list) {
    ...
    if (tz !== 'unknown') {
      ...
      tradingState = td.isNonTradingDay ? '非交易日'
        : cls === 'US' ? ({ PRE: '盘前交易', REGULAR: '交易中', POST: '盘后交易' }[usExtendedSession()])
        : !td.hasOpened ? '未开盘' : td.isClosed ? '已收盘' : '交易中'
    }
    ...
    out.set(stock.stockCode, {
      ...
      priceLine: buildPriceLine(q),
      calcLine: buildCalcLine(q),
    })
  }
  return out
})
