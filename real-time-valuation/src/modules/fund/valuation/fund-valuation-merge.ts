...
export async function batchGetValuation(
  ...
  await runConcurrent(fundCodes, FUND_VALUATION_CONFIG.BATCH_CONCURRENCY, async (code) => {
    const hit = preMap.get(code)
    const v = hit ? await getFundValuation(code, getFundType, hit) : await getFundValuation(code, getFundType)
    if (v) result.set(code, v)
  })
  return result
}
