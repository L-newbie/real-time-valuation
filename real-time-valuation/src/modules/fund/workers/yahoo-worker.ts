...
async function fetchYahooQuote(
  ...
  const timeout = mode === 'realtime' ? 4000 : 10000
  ...
  return {
    ...
    ...(mm?.regularMarketPrice != null && Number.isFinite(mm.regularMarketPrice) && mm.regularMarketPrice > 0 ? { price: mm.regularMarketPrice } : {}),
    ...(mm?.previousClose != null && Number.isFinite(mm.previousClose) && mm.previousClose > 0 ? { prevClose: mm.previousClose } : {}),
    proxyFailed: false,
    source: 'Yahoo',
  }
}
...
self.onmessage = async (e: MessageEvent<WorkerIncomingMessage>) => {
  ...
  if (type === 'yahoo-quote-batch') {
    ...
    let idx = 0
    const size = Math.min(BATCH_CONCURRENCY, entries.length)
