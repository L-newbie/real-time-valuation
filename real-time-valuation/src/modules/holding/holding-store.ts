...
import { getBeijingTodayStr, getNowStr, getPreviousNTradingDay, getPreviousBusinessTradingDay, getTradingDayFromToday } from '@/modules/fund/valuation/cn-trading-day'
...
export const useHoldingStore = defineStore('holding', () => {
  ...
  function cancelPendingAction(actionId: string): boolean {
    ...
    if (action.status !== PendingActionStatus.Pending) return false
  ...
  async function executePendingActions(valuationMap: Map<string, FundValuation>): Promise<void> {
    ...
    function resolveNav(action: PendingAction): { nav: number; navDate: string } | null {
      const v = valuationMap.get(action.fundCode)
      if (!v || v.isEstimated !== false || !v.jzrq || v.dwjz <= 0) return null
      const threshold = v.delayDays === 2 ? getPreviousBusinessTradingDay() : action.scheduledDate
      if (v.jzrq >= threshold) return { nav: v.dwjz, navDate: v.jzrq }
      return null
    }
    ...
    for (const action of pendingActions.value) {
      ...
      const resolved = resolveNav(action)
      if (!resolved) continue
      addHoldingByAmount(action.fundCode, action.amount, resolved.nav, action.note, resolved.navDate)
    ...
    await runConcurrent(Array.from(byFund.entries()), FUND_VALUATION_CONFIG.BATCH_CONCURRENCY, async ([, list]) => {
      try {
        ...
        for (const h of list) {
          ...
          if (!h.entryNavDate && Number.isFinite(h.confirmedBaseAmount) && h.confirmedBaseAmount! > 0
            && Math.abs(h.confirmedBaseAmount! - principal) > 0.005) {
            results.push(null)
            continue
          }
