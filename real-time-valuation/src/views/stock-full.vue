    ...
    <section class="watchlist-panel">
      ...
      <div v-else class="watchlist-grid">
        <div
          ...
          :key="item.code"
          v-memo="[item, isCardOpen(item.code)]"
        ...
        >
          ...
          <div class="wc-main">
            ...
            <span v-if="item.price > 0 && item.changeRate != null" :class="['wc-chg', watchRate(item)]">
            ...
            <span v-if="extShown(item)" class="wc-ext font-number" :class="extTone(item)">
              <span class="wc-ext-tag">{{ sessionLabel(item) }}</span>
              <span>{{ extRateFmt(item) }}</span>
            </span>
          </div>
...
const openCards = reactive<Record<string, boolean>>({})

function isCardOpen(code: string): boolean {
  return openCards[code] === true
}
...
const sortedWatchlist = computed<StockQuote[]>(() => {
  ...
  return rows.sort((a, b) => {
    ...
    return (b.changeRate ?? -Infinity) - (a.changeRate ?? -Infinity)
  })
})
...
function watchTint(w: StockQuote): string {
  if (w.price <= 0 || w.changeRate == null) return ''
...
function extShown(w: StockQuote): boolean {
  if (w.market !== 'US') return false
  if (w.extPrice == null || !Number.isFinite(w.extPrice)) return false
  if (w.extRate == null || !Number.isFinite(w.extRate)) return false
  return w.session === 'PRE' || w.session === 'POST'
}

function sessionLabel(w: StockQuote): string {
  return SESSION_CN[w.session ?? ''] ?? '盘外'
}

function extRateFmt(w: StockQuote): string {
  const r = w.extRate ?? 0
  return `${r >= 0 ? '+' : ''}${r.toFixed(2)}%`
}

function extTone(w: StockQuote): string {
  const r = w.extRate ?? 0
  return r > 0 ? 'text-rise' : r < 0 ? 'text-fall' : 'text-flat'
}
...
.wc-ext-tag { font-size: 9px; opacity: 0.7; letter-spacing: 0.02em; }
